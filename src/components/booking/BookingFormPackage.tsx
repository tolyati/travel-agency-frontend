...
export default function BookingFormPackage({ item, onClose, onConfirm }: Props) {
  ...
  const isValid = name.trim().length > 0 && phone.trim().length > 0 && date.length > 0 && guests >= 1;
  ...
  return (
    <BookingWrapper item={item} title="📦 Оформление пакета" submitted={submitted} onClose={onClose}>
